...
func (client *Client) CreateMailDraft(
	...
	if !validGmailID(response.Message.ID) ||
		!validGmailID(response.Message.HistoryID) {
...
func (client *Client) SendMail(
	...
	if !validGmailID(response.ID) || !validGmailID(response.HistoryID) {
...
func (client *Client) DeleteMail(
	...
	if _, err := decodeMessageID(input.MessageID); err != nil {
		return err
	}
	if _, err := decodeHistoryID(input.ChangeKey); err != nil {
		return err
	}
	return errors.New(
		"immediate permanent Gmail deletion is unavailable; move the message to Trash instead",
	)
}
