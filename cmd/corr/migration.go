...
func (app *runtime) migrateLegacyState(
	...
	if err != nil {
		return fmt.Errorf("migrate legacy v0.6 state: %w", err)
	}
	if migrated {
		_, err = fmt.Fprintln(
			app.stderr,
...
func (app *runtime) stopLegacyDaemon(
	...
	endpoint, err := localipc.ResolveLegacyInState(configPath, stateDirectory)
	if err != nil {
		return err
	}
	client, err := daemonapi.NewLegacyClient(endpoint)
	if err != nil {
		return err
	}
	...
	for {
		...
		if next.Status().ProcessID < 1 {
			...
			if nextErr == nil && daemonChanged(owner.Status(), next.Status()) {
				return errors.New(
					"a new v0.6 session owner started during migration; stop it and retry",
				)
			}
		}
		select {
